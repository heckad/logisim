...
package com.heckad.logisim.prefs;
...
import com.heckad.logisim.util.PropertyChangeWeakSupport;
    ...
    private static Preferences getPrefs(boolean shouldClear) {
        if (prefs == null) {
            synchronized(AppPreferences.class) {
                if (prefs == null) {
                    Preferences p = Preferences.userNodeForPackage(LogisimMain.class);
                    if (shouldClear) {
                        try { p.clear(); } catch (BackingStoreException e) { }
                    }
